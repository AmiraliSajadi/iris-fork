...
from src.codeql_queries import (CALL_QL_SUBSET_PREDICATE,
                                EXTENSION_SRC_SINK_YML_ENTRY,
                                EXTENSION_SUMMARY_YML_ENTRY,
                                EXTENSION_YML_TEMPLATE, QL_BODY_OR_SEPARATOR,
                                QL_FUNC_PARAM_NAME_ENTRY,
                                QL_FUNC_PARAM_SOURCE_ENTRY,
                                QL_METHOD_CALL_SOURCE_BODY_ENTRY,
                                QL_SINK_ARG_NAME_ENTRY, QL_SINK_ARG_THIS_ENTRY,
                                QL_SINK_BODY_ENTRY, QL_SINK_PREDICATE,
                                QL_SOURCE_PREDICATE, QL_STEP_PREDICATE,
                                QL_SUBSET_PREDICATE, QL_SUMMARY_BODY_ENTRY)
from src.config import (ALL_METHOD_INFO_DIR, CODEQL_DB_PATH, CODEQL_DIR,
                        CVES_MAPPED_W_COMMITS_DIR, OUTPUT_DIR,
                        PACKAGE_MODULES_PATH, PROJECT_SOURCE_CODE_DIR)
...
from src.prompts import (API_LABELLING_SYSTEM_PROMPT,
...
class SAPipeline:
    ...
    ################ My added function ####################
    ######################################################
    def _api_key(self, pkg, cls, mtd, sig):
        return f"{(pkg or '').strip()}::{(cls or '').strip()}::{(mtd or '').strip()}::{(sig or '').strip()}"

    def _canon_type(self, t: str) -> str:
        t = (t or "").strip()
        t = t.split("<", 1)[0]      # List<String> -> List
        t = t.replace("...", "[]")  # varargs -> array
        return t
    ...
    def _strip_file_uri(self, p: str) -> str:
        ...
        if p.startswith("file:"):
            u = urlparse(p)
            return unquote(u.path or p.replace("file://", ""))
        return p

    def _parse_location(self, loc: str):
        """'file:///.../Foo.java:171:30' -> ('/.../Foo.java', 171, 30)"""
        if not isinstance(loc, str) or not loc:
            return None, None, None
        s = self._strip_file_uri(loc)
        m = re.search(r":(\d+)(?::(\d+))?$", s)  # strip only trailing :line(:col)
        start = end = None
        if m:
            start = int(m.group(1))
    ...
    def _normalize_path_from_location(self, raw_path: str) -> str:
        ...
        if j != -1:
            tail = p[j + len(anchor):]
            cand = os.path.join(self.project_source_code_dir, tail)
            if os.path.exists(cand):
                return cand
        return os.path.join(self.project_source_code_dir, p.lstrip("/"))


    def _load_func_map(self):
    ...
    def _ensure_api_loc_index(self):
        if self._api_loc_index is not None:
            return
        try:
            df = self._load_candidates_df()
        except Exception:
            # Fallback: index from externals only
            df = pd.read_csv(self.external_apis_csv_path, keep_default_na=False)
            df = df.rename(columns={"clazz":"class","func":"method","full_signature":"signature"})
            if "location" not in df.columns:
                df["location"] = ""

        idx = defaultdict(list)
        for _, r in df.iterrows():
            pkg = r.get("package",""); cls = r.get("class",""); mtd = r.get("method","")
            sig = r.get("signature",""); loc = r.get("location","")
            k_exact = self._api_key(pkg, cls, mtd, sig)
    ...
    def retrieve_src(self, row):
        ...
        # prefer enclosing function block if available
        rel = os.path.relpath(file_path, self.project_source_code_dir)
        decls = self._func_map.get(rel, []) or self._func_map.get(file_path, [])
        if decls and start:
            enc = self.find_enclosing_declaration(start, (end or start), decls)
            if enc: _, start, end = enc

        sline = max(1, start or 1)
        eline = min(len(lines), (end or sline + 100))
        snippet = "".join(lines[sline-1:eline])

        header = f"// file: {rel}\n// lines: {sline}-{eline} (origin: {loc})\n"
        return header + snippet


    def process_unknown(self, unknown_apis):
        self.project_logger.info("==> Second pass: Re-querying GPT with source code context...")

        # Load prompt context
        system_prompt = API_LABELLING_WITH_SRC_SYSTEM_PROMPT
        ...
        def build_prompt_batch(i):
            batch = unknown_apis[i:i + batch_size]
            ...
            user_prompt = API_LABELLING_WITH_SRC_USER_PROMPT.format(
            ...
            with open(f"{self.label_api_log_path}/secondpass_prompt_{i}.txt", "w") as f:
                f.write(user_prompt + "\n")
    ...
    def query_gpt_for_api_src_tp_sink_batched(self):
        ...
        if not os.path.exists(self.llm_labelled_source_apis_path) or self.overwrite or self.overwrite_labelled_apis:
            ...
            known_apis = [r for r in merged_llm_results if r.get("type", "") in ("source", "sink", "taint-propagator")]
            unknown_apis = [r for r in merged_llm_results if r.get("type", "") == "unknown"]

            self.project_logger.info(f"  ==> First pass: {len(known_apis)} labeled, {len(unknown_apis)} unknown")
            ...
            # Combine all knowns from both passes
            final_apis = known_apis + processed_known

            # 7. Save the result for this project
            merged_overall_results = self.merge_llm_labeled_apis_and_cache(candidates, final_apis)
